...
ll_1 = readRDS("stan model outputs/logistic_fit.rds")
...
hut_mort_LL <- f_logistic(x = seq(0,1,length=101), LL_a_full[5], LL_c_full[5])
...
benefitpbo <- readRDS("stan model outputs/ento_pbo_benefit_Apr2022data.rds")
...
benefit_pp <- readRDS("stan model outputs/ento_g2_benefit_Apr2022data.rds") ## ALL
...
hut_mort_LLPP <- f_LOG_logistic_with_benefit(mort = seq(0,1,length=101),
                                             alp1 = fitbene_1a[5],
                                             alp2 = fitbene_2a[5])
...
resistance_ITN_default_params_2_f = function(product, 
  ...
  param_c = LL_c_full[data_picker_rand] 
  ...
  hut_mort_LL <- f_logistic(x = seq(0,1,length=101),#** 
                                param_a, param_c)
  ...
  mort_maxA   =  if(product == 0) f_logistic(x = 0,#** this is surv i.e. mort max when surv =0
                                param_a, param_c) else if(product == 1) f_LOG_logistic_with_benefit(mort = hut_mort_LL, 
                                                                                                    alp1 = fitbene_1[data_picker_rand],
...
# write.csv(pyrethroidOnlyNets,"parameters/logistic_assoc_pyrethroid_only_nets.csv")
# write.csv(pyrethroidPBONets,"parameters/logistic_assoc_pyrethroid_pbo_nets.csv")
# write.csv(ppNets,"parameters/logistic_assoc_pyrethroid_pyrrole_nets.csv")
